...
if(len(ws["zips"]) > 0):
    ...
    for z in ws["zips"]:
        ...
        if(len(allShps) == 1):
            if(len(shp) == 1):
                ...
                if(len(list(filter(lambda x: x[-4:] == '.shx', bZip.namelist()))) < 1):
                    gbHelpers.logWrite(ws, "CRITICAL ERROR: A valid *.shp requires a *.shx (index) file. None was found in " + z)
                ...
                if(len(list(filter(lambda x: x[-4:] == '.dbf', bZip.namelist()))) < 1):
                    gbHelpers.logWrite(ws, "CRITICAL ERROR: A valid *.shp requires a *.dbf (index) file. None was found in " + z)
                ...
                if(len(list(filter(lambda x: x[-4:] == '.prj', bZip.namelist()))) < 1):
                    gbHelpers.logWrite(ws, "CRITICAL ERROR: A valid *.shp requires a *.prj (index) file. None was found in " + z)
        ...
        if(checkFail == 1):
            ...
            gbHelpers.logWrite(ws, "CRITICAL ERROR: Zipfile validity checks failed for " + z + ".  Check the log to see what is wrong.")
        else:
