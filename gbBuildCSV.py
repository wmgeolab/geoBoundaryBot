...
ws = {}
ws["working"] = os.environ.get("GITHUB_WORKSPACE") or "/home/dan/git/gbRelease"
ws["logPath"] = os.path.expanduser("~") + "/tmp/log.txt"

# Load in the ISO lookup table
isoDetails = pd.read_csv(ws["working"] + "/geoBoundaryBot/dta/iso_3166_1_alpha_3.csv", encoding="utf-8")
...
for (path, dirname, filenames) in os.walk(ws["working"] + "/releaseData/"):
    ...
    if len(metaSearch) == 1:
        ...
        metaLine = (
            metaLine
            + meta["boundarySource-1"]
            + '","'
            + meta["boundarySource-2"]
            + '","'
        ...
        try:
            ...
            if len(vertices) == 0:
                ...
                metaLine = (
                    ...
                    + str(admCount)
                    + '","'
                    + str(round(sum(vertices) / len(vertices), 0))
                    + '","'
                    + str(min(vertices))
                    + '","'
                    + str(max(vertices))
        ...
        try:
            ...
            metaLine = (
                metaLine
                + str(lengthGeom["length"].mean())
                + '","'
                + str(lengthGeom["length"].min())
                + '","'
                + str(lengthGeom["length"].max())
                + '","'
            )
        except:
            metaLine = metaLine + "" + '","' + "" + '","' + "" + '","'
        ...
        try:
            ...
            metaLine = (
                metaLine
                + str(areaGeom["area"].mean())
                + '","'
                + str(areaGeom["area"].min())
                + '","'
                + str(areaGeom["area"].max())
                + '","'
            )
        except:
            metaLine = metaLine + "" + '","' + "" + '","' + "" + '","'
